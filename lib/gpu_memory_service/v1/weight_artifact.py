...
from gpu_memory_service.snapshot.transfer import GMSSnapshotConfig as GMSTransferConfig
...
def hydrate_weights(
    ...
    *,
    ...
    vmm.runtime_set_device(device)
    granularity = int(vmm.get_allocation_granularity(device))
    manifest, sources = _load_manifest(artifact_dir)
    backend = create_transfer_backend(
        TransferBackendKind.NIXL.value,
        GMSTransferConfig(device=device, max_workers=max_workers),
    )
    with _Cleanup() as resources:
...
def _write_shards(
    ...
    allocations: list[WeightArtifactAllocation] = []
