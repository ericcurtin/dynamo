...
impl GroupedLiveActor {
    ...
    async fn apply_control(&mut self, envelope: ControlEnvelope) -> Result<()> {
        ...
        let is_request_cancellation = matches!(
...
mod tests {
    ...
    async fn cancellation_does_not_end_a_pass_with_unrelated_pending_output() {
        ...
        let cancelled = handle.apply_command(SchedulerCommand::new(
            0,
            NativeCommand::Submit(request(31, 4, 2)),
        ));
        let unrelated = handle.apply_command(SchedulerCommand::new(
            0,
            NativeCommand::Submit(request(32, 4, 1)),
        ));
