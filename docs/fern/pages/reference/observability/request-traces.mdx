...
`OTEL_TRACES_SAMPLE_RATIO` also applies to request-trace records before fan-out. Dynamo uses the
agent session ID for agent request and tool rows, and a stable request ID for other request-end and
payload rows. A selected record is written to every configured sink and a rejected record is written
to none. Legacy payload capture enabled by `DYN_AUDIT_FORCE_LOGGING` remains unsampled.
