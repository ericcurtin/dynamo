...
`OTEL_TRACES_SAMPLE_RATIO` also applies to request-trace records before fan-out. Dynamo uses a
stable request ID for request-end and payload rows, and the harness session ID for tool rows, so a
selected record is written to every configured sink and a rejected record is written to none.
