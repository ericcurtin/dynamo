...
| `OTEL_TRACES_SAMPLE_RATIO` | unset | Optional `[0.0, 1.0]` ratio shared with runtime spans and request traces. It filters FPM rows after mode and interval selection, using a stable source, worker, rank, and counter key. |
...
When `OTEL_TRACES_SAMPLE_RATIO` is set, Dynamo filters selected FPM rows after this mode-specific
selection. It does not change `DYN_FPM_MODE` or `DYN_FPM_SAMPLE_INTERVAL_MS`.
