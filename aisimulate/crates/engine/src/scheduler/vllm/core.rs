...
use crate::native::{
    HandoffId, NativePressureEvent, NativePressureKind, NativePressureState, modeled_duration_ms,
};
#[cfg(test)]
...
pub(crate) struct VllmCore {
    ...
    pressure_events: Vec<NativePressureEvent>,
...
impl VllmCore {
    ...
    pub(super) fn policy_preempt(&mut self, at_ms: f64) -> Option<PreemptedRequest> {
        ...
        if let Some(preempted) = preempted.as_ref() {
            ...
            self.pressure_events.push(NativePressureEvent {
                at_ms,
    ...
    fn pressure_state(&self) -> NativePressureState {
        NativePressureState {
