...
use aisimulate_engine::{
    NativeCommandResult, NativeKvEvent, NativeLifecycleEvent, NativePressureEvent,
};
...
pub trait ReplayEngineObservation {
    ...
    /// Whether this observation adapter needs native KV events for one worker
    ...
    fn capture_native_kv_events(_stage: crate::WorkerStage) -> bool {
        Self::CAPTURE_NATIVE_KV_EVENTS
    }
...
pub(crate) struct PressureEvent {
    ...
    pub(crate) event: NativePressureEvent,
}
