...
use crate::{
    ReplayCaptureOptions, ReplayDeterminism, ReplayError, ReplayResult, ReplaySpec, ReplayTopology,
    SlaThresholds, TraceSimulationReport, WorkerStage,
};
        ...
        let collector = match &self.spec.topology {
            ReplayTopology::Aggregated { workers } => {
                let role_factory = self.factory.role_factory(
                    ...
                    C::Observation::capture_native_kv_events(WorkerStage::Aggregated)
                        || artifact_sink.is_some(),
                )?;
                let startup_time_ms = positive_delay(workers.startup_delay_ms);
                let input_supports_single_worker_runtime = match &runtime_input {
                    ReplayRuntimeInput::Requests(pending) => pending
                        .iter()
                        .all(|request| request.preferred_dp_rank.is_none()),
                    ReplayRuntimeInput::Workload(_) => true,
                };

                let use_single_worker_runtime = workers.initial_workers == 1
                    && role_factory.dp_size() == 1
                    && scaling.is_none()
                    && !self.capture.capture_lifecycle_evidence
                    && !self.capture.capture_canonical_evidence
                    && input_supports_single_worker_runtime
            ...
            ReplayTopology::Disaggregated {
                ...
                let prefill_factory = self.factory.role_factory(
                    &engine_config,
                    WorkerStage::Prefill,
                    C::Observation::capture_native_kv_events(WorkerStage::Prefill),
                )?;
                let decode_factory = self.factory.role_factory(
                    &engine_config,
                    WorkerStage::Decode,
                    C::Observation::capture_native_kv_events(WorkerStage::Decode),
                )?;
