...
use aisimulate_replay::{
    ...
    NoReplayMetadata, PoolRoundRobinPlacement, ProviderSpec, ReplayAdapters, ReplayCaptureOptions,
    ReplayComposition, ReplayDeterminism, ReplayRequest, ReplayScalingDecision,
    ReplayScalingPolicy, ReplayScalingSnapshot, ReplaySpec, ReplayTopology, Replayer,
    TraceSimulationReport, WorkerPoolSpec, WorkerTopology,
};
...
fn run_fast_aggregated(spec: ReplaySpec) -> TraceSimulationReport {
    Replayer::new(spec, NativeReplayFactory::new())
        .unwrap()
        .with_capture_options(canonical_options())
        .run()
        .unwrap()
...
fn single_worker_fast_path_matches_general_aggregated_runtime() {
    ...
    for backend in [
        ...
        let fast_path = run_fast_aggregated(spec.clone());
...
fn authored_rank_is_validated_by_the_default_aggregated_runtime() {
    ...
    let default_error = Replayer::new(spec.clone(), NativeReplayFactory::new())
        .unwrap()
        .with_capture_options(canonical_options())
        .run()
        .unwrap_err()
...
fn max_sim_time_is_the_same_soft_cap_for_fast_and_general_aggregated_runtimes() {
    ...
    let fast_path = run_fast_aggregated(capped.clone());
