...
use aisimulate_replay::{
    AggregatedRoundRobinPlacement, NativeReplayEngineConfig, NativeReplayFactory,
    NativeReplayRoleConfig, NoEngineEvents, NoReplayMetadata, PoolRoundRobinPlacement,
...
fn run_canonical_native_replay(spec: ReplaySpec) -> TraceSimulationReport {
...
fn built_in_aggregated_replay_produces_a_deterministic_report() {
    ...
    let first = run_canonical_native_replay(spec.clone());
    let second = run_canonical_native_replay(spec);
...
fn multi_worker_round_robin_uses_each_logical_worker_deterministically() {
    ...
    let first = run_canonical_native_replay(replay.clone());
    let second = run_canonical_native_replay(replay);
    assert_deterministic(&first, &second);
