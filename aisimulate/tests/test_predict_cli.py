...
def test_missing_dynamo_stack_recommends_planner_local_install(monkeypatch):
    ...
    with pytest.raises(RunnerUnavailableError, match="dynamo-planner") as error:
        cli._default_runner_factory("dynamo")

    assert "uv pip install --no-deps -e ." in str(error.value)
    assert "container/deps/requirements.planner.txt" in str(error.value)
