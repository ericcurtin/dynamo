...
struct cuda_api {
  ...
  CUresult(CUDAAPI* granularity)(size_t*, const CUmemAllocationProp*, CUmemAllocationGranularity_flags);
  CUresult(CUDAAPI* reserve)(CUdeviceptr*, size_t, size_t, CUdeviceptr, unsigned long long);
  CUresult(CUDAAPI* address_free)(CUdeviceptr, size_t);
  CUresult(CUDAAPI* create)(CUmemGenericAllocationHandle*, size_t, const CUmemAllocationProp*, unsigned long long);
  CUresult(CUDAAPI* import_handle)(CUmemGenericAllocationHandle*, void*, CUmemAllocationHandleType);
  CUresult(CUDAAPI* export_handle)(void*, CUmemGenericAllocationHandle, CUmemAllocationHandleType, unsigned long long);
  CUresult(CUDAAPI* release)(CUmemGenericAllocationHandle);
  CUresult(CUDAAPI* map)(CUdeviceptr, size_t, size_t, CUmemGenericAllocationHandle, unsigned long long);
  CUresult(CUDAAPI* unmap)(CUdeviceptr, size_t);
  CUresult(CUDAAPI* set_access)(CUdeviceptr, size_t, const CUmemAccessDesc*, size_t);
...
static void
check_cuda(CUresult result, const char* operation)
{
...
static void*
required_symbol(void* library, const char* name)
{
...
static void
fill_pattern(unsigned char* buffer, size_t size, unsigned int seed)
{
  size_t index;
  for (index = 0; index < size; index++) buffer[index] = (unsigned char)(index * 131U + seed);
}

static void
send_app(int fd, const struct app_message* message, int passed_fd)
{
  struct iovec iov = {
  ...
  require(sendmsg(fd, &envelope, MSG_NOSIGNAL) == sizeof(*message), "send app message");
}

static struct app_message
receive_app(int fd, int* received_fd)
{
  ...
  *received_fd = -1;
  require(
      recvmsg(fd, &envelope, MSG_CMSG_CLOEXEC) == sizeof(message) &&
...
static size_t
allocation_size(struct cuda_api* cuda, CUmemAllocationProp* properties)
{
  size_t granularity;
  check_cuda(cuda->granularity(&granularity, properties, CU_MEM_ALLOC_GRANULARITY_RECOMMENDED), "granularity");
  return (DATA_SIZE + granularity - 1) / granularity * granularity;
}

static void
owner_worker(int control_fd, int peer_fd)
{
...
static void
importer_worker(int control_fd, int peer_fd)
{
  struct cuda_api cuda = load_cuda();
  CUmemAllocationProp properties = {0};
  CUmemAccessDesc access = {
  ...
  require(message.command == APP_INITIAL_FD && import_fd >= 0, "importer initial FD");
  size = message.size;
  check_cuda(cuda.init(0), "init");
  check_cuda(cuda.device_get(&device, 1), "device 1");
  check_cuda(cuda.context_create(&context, 0, device), "importer context");
  properties.type = CU_MEM_ALLOCATION_TYPE_PINNED;
  properties.location.type = CU_MEM_LOCATION_TYPE_DEVICE;
  properties.location.id = device;
  properties.requestedHandleTypes = CU_MEM_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR;
  ...
  check_cuda(cuda.set_access(imported_address, size, &access, 1), "import access");
  check_cuda(cuda.create(&local, size, &properties, 0), "local create");
  check_cuda(cuda.reserve(&local_address, size, 0, 0, 0), "local reserve");
  check_cuda(cuda.map(local_address, size, 0, local, 0), "local map");
  check_cuda(cuda.set_access(local_address, size, &access, 1), "local access");
  pattern = malloc(size);
  require(pattern != NULL, "importer pattern");
  fill_pattern(pattern, size, 83);
  check_cuda(cuda.copy_htod(local_address, pattern, size), "local pattern copy");
  message = (struct app_message){
      .command = APP_READY,
  ...
  for (;;) {
    ...
    if (message.command == APP_PEER_WRITE) {
      check_cuda(cuda.copy_dtod(imported_address, local_address, size), "peer DtoD write");
      message.status = 0;
    } else if (message.command == APP_CLEANUP) {
      check_cuda(cuda.unmap(imported_address, size), "imported unmap");
      check_cuda(cuda.release(imported), "imported release");
      check_cuda(cuda.address_free(imported_address, size), "imported address free");
      check_cuda(cuda.unmap(local_address, size), "local unmap");
      check_cuda(cuda.release(local), "local release");
      check_cuda(cuda.address_free(local_address, size), "local address free");
      check_cuda(cuda.context_destroy(context), "importer context destroy");
      message.status = 0;
      send_app(control_fd, &message, -1);
      break;
    } else {
...
connect_agent(pid_t pid)
{
  ...
  require(
      snprintf(address.sun_path, sizeof(address.sun_path), "%s/%s%d.sock", directory, DYN_VMM_SOCKET_PREFIX, pid) <
...
agent_command(
    pid_t pid, uint16_t operation, uint64_t generation, const struct dyn_vmm_packet* fields, int passed_fd,
    int* received_fd, bool expect_success)
{
  ...
  require(
      sendmsg(fd, &send_message, MSG_NOSIGNAL) == sizeof(packet) &&
          recvmsg(fd, &receive_message, MSG_CMSG_CLOEXEC) == sizeof(packet),
      "agent exchange");
  if (CMSG_FIRSTHDR(&receive_message) != NULL)
    memcpy(&result_fd, CMSG_DATA(CMSG_FIRSTHDR(&receive_message)), sizeof(result_fd));
  close(fd);
  require((packet.status == 0) == expect_success, expect_success ? packet.message : "agent unexpectedly succeeded");
  if (received_fd != NULL)
...
static void
audit_owner(pid_t pid, uint64_t generation, uint64_t* dev, uint64_t* ino)
{
  struct dyn_vmm_packet packet = {
      .magic = DYN_VMM_MAGIC,
      .version = DYN_VMM_VERSION,
  ...
  require(
      send(fd, &packet, sizeof(packet), MSG_NOSIGNAL) == sizeof(packet) &&
          recv(fd, &packet, sizeof(packet), MSG_WAITALL) == sizeof(packet) && packet.status == 0,
      "pair audit");
  *dev = 0;
  *ino = 0;
  for (index = 0; index < packet.count; index++) {
    struct dyn_vmm_packet record;
    require(recv(fd, &record, sizeof(record), MSG_WAITALL) == sizeof(record), "pair audit record");
    if (record.role == DYN_VMM_ROLE_OWNER && record.object_dev != 0) {
...
static pid_t
spawn_worker(const char* self, const char* role, int control_fd, int peer_fd, const char* interposer)
{
  pid_t pid = fork();
  ...
  if (pid == 0) {
    ...
    require(fcntl(control_fd, F_SETFD, 0) == 0 && fcntl(peer_fd, F_SETFD, 0) == 0, "clear worker descriptor CLOEXEC");
...
static void
app_roundtrip(int fd, uint32_t command, const char* detail)
{
  struct app_message message = {
      .command = command,
  };
  ...
  if (detail != NULL)
    strncpy(message.detail, detail, sizeof(message.detail) - 1);
  send_app(fd, &message, -1);
  message = receive_app(fd, &received_fd);
  require(received_fd < 0 && message.status == 0, "worker command failed");
}

static int
controller(const char* self, const char* interposer)
{
  int owner_control[2];
  ...
  require(
      socketpair(AF_UNIX, SOCK_SEQPACKET, 0, owner_control) == 0 &&
          socketpair(AF_UNIX, SOCK_SEQPACKET, 0, importer_control) == 0 &&
          socketpair(AF_UNIX, SOCK_SEQPACKET, 0, peer) == 0,
      "worker socketpairs");
  {
    int all_fds[] = {
        owner_control[0], owner_control[1], importer_control[0], importer_control[1], peer[0], peer[1],
    };
    size_t index;
    for (index = 0; index < sizeof(all_fds) / sizeof(all_fds[0]); index++)
      require(fcntl(all_fds[index], F_SETFD, FD_CLOEXEC) == 0, "set launcher descriptor CLOEXEC");
  }
  owner_pid = spawn_worker(self, "owner", owner_control[1], peer[0], interposer);
  importer_pid = spawn_worker(self, "importer", importer_control[1], peer[1], interposer);
  ...
  audit_owner(owner_pid, generation, &object.object_dev, &object.object_ino);
  {
    ...
    audit_owner(importer_pid, generation, &ignored_dev, &ignored_ino);
  }
  (void)agent_command(owner_pid, DYN_VMM_OP_SYNC_ALL_CONTEXTS, generation, NULL, -1, NULL, true);
  (void)agent_command(importer_pid, DYN_VMM_OP_SYNC_ALL_CONTEXTS, generation, NULL, -1, NULL, true);
  (void)agent_command(owner_pid, DYN_VMM_OP_DETACH_IMPORTS, generation, NULL, -1, NULL, true);
  (void)agent_command(importer_pid, DYN_VMM_OP_DETACH_IMPORTS, generation, NULL, -1, NULL, true);
  (void)agent_command(owner_pid, DYN_VMM_OP_VERIFY_LOCK_READY, generation, NULL, -1, NULL, true);
  (void)agent_command(importer_pid, DYN_VMM_OP_VERIFY_LOCK_READY, generation, NULL, -1, NULL, true);
  exported = agent_command(owner_pid, DYN_VMM_OP_EXPORT, generation, &object, -1, &fresh_fd, true);
  require(fresh_fd >= 0, "pair fresh FD absent");
  (void)agent_command(importer_pid, DYN_VMM_OP_IMPORT, generation, &exported, fresh_fd, NULL, true);
  close(fresh_fd);
  (void)agent_command(owner_pid, DYN_VMM_OP_VERIFY_ACTIVE, generation, NULL, -1, NULL, true);
  (void)agent_command(importer_pid, DYN_VMM_OP_VERIFY_ACTIVE, generation, NULL, -1, NULL, true);
  (void)agent_command(owner_pid, DYN_VMM_OP_COMMIT, generation, NULL, -1, NULL, true);
  (void)agent_command(importer_pid, DYN_VMM_OP_COMMIT, generation, NULL, -1, NULL, true);
  ...
  require(waitpid(owner_pid, &status, 0) == owner_pid && WIFEXITED(status) && WEXITSTATUS(status) == 0, "owner exit");
  require(
      waitpid(importer_pid, &status, 0) == importer_pid && WIFEXITED(status) && WEXITSTATUS(status) == 0,
      "importer exit");
  fprintf(
      stdout,
