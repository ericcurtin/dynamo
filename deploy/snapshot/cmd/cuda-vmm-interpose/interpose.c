...
typedef CUresult(CUDAAPI* map_type)(CUdeviceptr, size_t, size_t, CUmemGenericAllocationHandle, unsigned long long);
typedef CUresult(CUDAAPI* unmap_type)(CUdeviceptr, size_t);
typedef CUresult(CUDAAPI* release_type)(CUmemGenericAllocationHandle);
typedef CUresult(CUDAAPI* import_type)(CUmemGenericAllocationHandle*, void*, CUmemAllocationHandleType);
...
static void
maybe_remove_owner(struct owner_allocation* owner)
{
  struct owner_allocation** cursor;

  if (owner == NULL || !owner->application_released || owner->mapped_ptr != 0)
    return;
  cursor = &owners;
  while (*cursor != NULL) {
    if (*cursor == owner) {
...
static void
mark_unsupported(uint32_t flag, const char* reason)
{
  unsupported |= flag;
...
static int
fd_identity(int fd, uint64_t* dev, uint64_t* ino)
{
  struct stat status;
...
static int
find_loader_symbols(struct dl_phdr_info* info, size_t size, void* data)
{
  ...
  if (info->dlpi_name == NULL ||
      (strstr(info->dlpi_name, "/libc.so.") == NULL && strstr(info->dlpi_name, "/libdl.so.") == NULL))
    return 0;
  ...
  for (index = 0; index < hash[1]; index++) {
    ...
    if (symbol->st_shndx == SHN_UNDEF || ELF64_ST_BIND(symbol->st_info) == STB_LOCAL)
      continue;
    for (wanted = 0; wanted < 3; wanted++) {
      if (symbols->found[wanted] == NULL && strcmp(string_table + symbol->st_name, symbols->wanted[wanted]) == 0)
        symbols->found[wanted] = (void*)(info->dlpi_addr + symbol->st_value);
    }
  }
  return symbols->found[0] != NULL && symbols->found[1] != NULL && symbols->found[2] != NULL ? 1 : 0;
}
...
static enum explicit_cuda_loader
classify_explicit_cuda_handle(void* handle, const char** path)
{
...
static bool
retain_explicit_cuda_handle(enum explicit_cuda_loader loader, const char* path)
{
  void** slot;
  void* retained;
  bool available;
  ...
  if (loader == EXPLICIT_CUDA_DEFAULT)
    return true;
  slot = loader == EXPLICIT_CUDA_DRIVER ? &explicit_libcuda : &explicit_libcudart;
...
__attribute__((used, noinline)) static struct loader_lookup
intercept_dlsym(void* handle, const char* symbol)
{
...
__attribute__((used, noinline)) static struct loader_lookup
intercept_dlvsym(void* handle, const char* symbol, const char* version)
{
  struct loader_lookup lookup = {0};
  void* result;
  void* replacement;
...
__attribute__((naked, visibility("default"))) void*
dlvsym(void*, const char*, const char*)
{
  __asm__ volatile(
      "push %rdi\n"
      "push %rsi\n"
...
static bool
is_synthetic(CUmemGenericAllocationHandle handle)
{
  return (handle & SYNTHETIC_MASK) == SYNTHETIC_TAG;
}

static struct logical_handle*
...
static struct owner_allocation*
find_owner(CUmemGenericAllocationHandle real)
{
  struct owner_allocation* owner;

  for (owner = owners; owner != NULL; owner = owner->next) {
    if (!owner->application_released && owner->application_real == real)
      return owner;
  }
  return NULL;
}
...
static struct owner_allocation*
find_owner_by_id(uint64_t dev, uint64_t ino)
{
  struct owner_allocation* owner;

  for (owner = owners; owner != NULL; owner = owner->next) {
    if (owner->exported && owner->object_dev == dev && owner->object_ino == ino)
      return owner;
  }
  return NULL;
}

static struct imported_allocation*
find_import(uint64_t dev, uint64_t ino)
{
  struct imported_allocation* allocation;

  for (allocation = imports; allocation != NULL; allocation = allocation->next) {
    if (allocation->object_dev == dev && allocation->object_ino == ino)
      return allocation;
  }
  return NULL;
}
...
find_reservation(CUdeviceptr ptr, size_t size)
{
  struct reservation* reservation;

  if (ptr > UINT64_MAX - size)
    return NULL;
  for (reservation = reservations; reservation != NULL; reservation = reservation->next) {
    if (ptr >= reservation->ptr && ptr + size <= reservation->ptr + reservation->size)
      return reservation;
  }
  return NULL;
}

static struct imported_mapping*
find_mapping_covering(CUdeviceptr ptr)
{
...
static CUresult
translate_import(
    CUmemGenericAllocationHandle logical, CUmemGenericAllocationHandle* real, struct imported_allocation** allocation)
{
...
static void
free_access(struct access_record* access)
{
  while (access != NULL) {
...
static bool
import_is_referenced(struct imported_allocation* allocation)
{
  struct logical_handle* handle;
  struct imported_mapping* mapping;
...
static void
maybe_remove_import(struct imported_allocation* allocation)
{
...
enter_context(CUcontext context, struct context_scope* scope)
{
  ...
  if (context == NULL || get_function == NULL || get_function(&scope->previous) != CUDA_SUCCESS)
    return -1;
  if (scope->previous == context)
    return 0;
  if (set_function == NULL || set_function(context) != CUDA_SUCCESS)
    return -1;
  scope->changed = true;
  return 0;
}
...
static int
leave_context(struct context_scope* scope)
{
  typedef CUresult(CUDAAPI * set_type)(CUcontext);
  set_type set_function = (set_type)resolve_next("cuCtxSetCurrent");
  ...
  if (!scope->changed)
    return 0;
  return set_function != NULL && set_function(scope->previous) == CUDA_SUCCESS ? 0 : -1;
}
...
current_context(CUcontext* context)
{
  ...
  function_type function = (function_type)resolve_next("cuCtxGetCurrent");

  return function != NULL && function(context) == CUDA_SUCCESS && *context != NULL ? 0 : -1;
}

static int
synchronize_context(CUcontext context)
{
  typedef CUresult(CUDAAPI * function_type)(void);
  function_type function = (function_type)resolve_next("cuCtxSynchronize");
...
static int
scoped_map(map_type function, struct imported_mapping* mapping, CUmemGenericAllocationHandle real)
{
  struct context_scope scope;
  int result;

  if (function == NULL || enter_context(mapping->context, &scope) != 0)
    return -1;
  result = function(mapping->ptr, mapping->size, mapping->offset, real, 0) == CUDA_SUCCESS ? 0 : -1;
  if (leave_context(&scope) != 0)
    return -1;
  return result;
}
...
static int
scoped_unmap(unmap_type function, struct imported_mapping* mapping)
{
  struct context_scope scope;
  int result;
  ...
  if (function == NULL || enter_context(mapping->context, &scope) != 0)
    return -1;
  result = function(mapping->ptr, mapping->size) == CUDA_SUCCESS ? 0 : -1;
  if (leave_context(&scope) != 0)
    return -1;
  return result;
}
...
static int
restore_access(struct imported_mapping* mapping)
{
  set_access_type function = (set_access_type)resolve_next("cuMemSetAccess");
  struct access_record* access;
  struct context_scope scope;
  int result = 0;
  ...
  if (function == NULL || enter_context(mapping->context, &scope) != 0)
    return -1;
  for (access = mapping->access; access != NULL; access = access->next) {
    if (function(access->ptr, access->size, access->descriptors, access->descriptor_count) != CUDA_SUCCESS) {
      result = -1;
      break;
    }
  }
...
effective_access(struct imported_mapping* mapping, CUdeviceptr ptr, const CUmemLocation* location)
{
  struct access_record* access;
  ...
  for (access = mapping->access; access != NULL; access = access->next) {
    ...
    for (index = 0; index < access->descriptor_count; index++) {
      if (access->descriptors[index].location.type == location->type &&
...
static int
verify_access(struct imported_mapping* mapping)
{
  typedef CUresult(CUDAAPI * function_type)(unsigned long long*, const CUmemLocation*, CUdeviceptr);
  function_type function = (function_type)resolve_next("cuMemGetAccess");
  struct access_record* access;
  struct context_scope scope;
  int result = 0;
  ...
  for (access = mapping->access; access != NULL; access = access->next) {
    ...
    for (index = 0; index < access->descriptor_count; index++) {
      ...
      CUmemLocation* location = &access->descriptors[index].location;
      if (function(&first_flags, location, access->ptr) != CUDA_SUCCESS ||
          function(&last_flags, location, last) != CUDA_SUCCESS ||
          first_flags != effective_access(mapping, access->ptr, location) ||
          last_flags != effective_access(mapping, last, location)) {
        result = -1;
        goto done;
      }
    }
...
static void
packet_error(struct dyn_vmm_packet* packet, const char* format, ...)
{
  va_list args;
...
static uint32_t
mapping_count(struct imported_allocation* allocation)
{
  struct imported_mapping* mapping;
  uint32_t count = 0;
...
static void
set_packet_state(struct dyn_vmm_packet* packet)
{
  ...
  if (phase == DYN_VMM_PHASE_LOCK_READY || phase == DYN_VMM_PHASE_REATTACHING)
...
static int
send_packet(int fd, const struct dyn_vmm_packet* packet, int passed_fd)
{
  struct iovec iov = {
  ...
  return sendmsg(fd, &message, MSG_NOSIGNAL) == (ssize_t)sizeof(*packet) ? 0 : -1;
}

static int
receive_packet(int fd, struct dyn_vmm_packet* packet, int* received_fd)
{
  struct iovec iov = {
      .iov_base = packet,
      .iov_len = sizeof(*packet),
  ...
  *received_fd = -1;
  ...
  for (header = CMSG_FIRSTHDR(&message); header != NULL; header = CMSG_NXTHDR(&message, header)) {
    ...
    if (header->cmsg_level != SOL_SOCKET || header->cmsg_type != SCM_RIGHTS || payload == 0 ||
  ...
  if (size != (ssize_t)sizeof(*packet) || (message.msg_flags & (MSG_TRUNC | MSG_CTRUNC)) != 0)
    invalid = true;
  if (!invalid)
    return 0;
  if (*received_fd >= 0) {
...
static int
audit_local(struct dyn_vmm_packet* response)
{
  struct imported_allocation* allocation;
  struct owner_allocation* owner;

  if (fork_child)
    mark_unsupported(DYN_VMM_UNSUPPORTED_FORK, "CUDA VMM state was inherited across fork; exec is required");
  if (unsupported != 0 || phase == DYN_VMM_PHASE_POISONED) {
    packet_error(
        response, "%s%sunsupported CUDA state flags=0x%x", poison_reason[0] != '\0' ? poison_reason : "",
  ...
  for (owner = owners; owner != NULL; owner = owner->next) {
    if (owner->exported) {
      if (owner->application_released && owner->mapped_ptr == 0) {
        packet_error(response, "released exported owner has no native mapping to retain");
        return -1;
      }
      response->count++;
    }
...
static int
handle_audit(const struct dyn_vmm_packet* request, int client, struct dyn_vmm_packet* response)
{
  struct imported_allocation* allocation;
  struct owner_allocation* owner;
  ...
  if (phase == DYN_VMM_PHASE_ACTIVE && generation == 0 && request->generation != 0) {
...
static bool
context_seen_before_owner(struct owner_allocation* owner)
{
...
static bool
context_seen_before_mapping(struct imported_mapping* mapping)
{
  struct owner_allocation* owner;
...
static int
handle_sync(const struct dyn_vmm_packet* request, struct dyn_vmm_packet* response)
{
  struct owner_allocation* owner;
  struct imported_mapping* mapping;

  if (request->generation != generation || (phase != DYN_VMM_PHASE_AUDITED && phase != DYN_VMM_PHASE_CONTEXTS_SYNCED)) {
  ...
  if (phase == DYN_VMM_PHASE_CONTEXTS_SYNCED)
    return 0;
  for (owner = owners; owner != NULL; owner = owner->next) {
    if (!owner->exported || context_seen_before_owner(owner))
      continue;
...
static CUmemGenericAllocationHandle
allocation_real(struct imported_allocation* allocation)
{
  struct logical_handle* handle;
...
static struct imported_mapping*
active_mapping(struct imported_allocation* allocation)
{
  struct imported_mapping* mapping;
...
static int
scoped_retain(retain_type function, struct imported_mapping* mapping, CUmemGenericAllocationHandle* real)
{
  struct context_scope scope;
  int result;
  ...
  if (function == NULL || enter_context(mapping->context, &scope) != 0)
    return -1;
  result = function(real, (void*)(uintptr_t)mapping->ptr) == CUDA_SUCCESS ? 0 : -1;
  if (leave_context(&scope) != 0)
    return -1;
  return result;
}

static enum remap_result
best_effort_remap_detached(void)
{
  map_type map_function = (map_type)resolve_next("cuMemMap");
  retain_type retain_function = (retain_type)resolve_next("cuMemRetainAllocationHandle");
  struct imported_mapping* mapping;
  struct logical_handle* handle;
  ...
  for (handle = handles; handle != NULL; handle = handle->next) {
    ...
    if (scoped_retain(retain_function, source, &retained) != 0)
      return REMAP_FAILED;
...
static int
handle_detach(const struct dyn_vmm_packet* request, struct dyn_vmm_packet* response)
{
  unmap_type unmap_function = (unmap_type)resolve_next("cuMemUnmap");
  release_type release_function = (release_type)resolve_next("cuMemRelease");
  struct imported_mapping* mapping;
  struct logical_handle* handle;

  if (request->generation != generation ||
      (phase != DYN_VMM_PHASE_CONTEXTS_SYNCED && phase != DYN_VMM_PHASE_LOCK_READY)) {
  ...
  for (mapping = mappings; mapping != NULL; mapping = mapping->next) {
    ...
    if (scoped_unmap(unmap_function, mapping) != 0) {
      ...
      packet_error(response, "failed to unmap peer VA 0x%llx", (unsigned long long)mapping->ptr);
      return -1;
    }
    mapping->detached = true;
    stats.peer_detached++;
    log_message(
        "event=peer_detach generation=%llu va=0x%llx size=%zu offset=%zu", (unsigned long long)generation,
        (unsigned long long)mapping->ptr, mapping->size, mapping->offset);
  }
  for (handle = handles; handle != NULL; handle = handle->next) {
    if (handle->real == 0)
      continue;
    if (release_function == NULL || release_function(handle->real) != CUDA_SUCCESS) {
      enum remap_result rollback = best_effort_remap_detached();
      ...
      if (rollback == REMAP_FAILED) {
        ...
        revision++;
        packet_error(
            response, rollback == REMAP_COMPLETE ? "failed to release imported real handle; "
...
static int
handle_verify_lock_ready(const struct dyn_vmm_packet* request, struct dyn_vmm_packet* response)
{
  struct imported_mapping* mapping;
  struct logical_handle* handle;

  if (request->generation != generation || phase != DYN_VMM_PHASE_LOCK_READY) {
    packet_error(response, "lock-ready generation/state mismatch");
    return -1;
  }
  for (mapping = mappings; mapping != NULL; mapping = mapping->next) {
    if (!mapping->detached || find_reservation(mapping->ptr, mapping->size) == NULL ||
        mapping->reservation_ptr > mapping->ptr ||
        mapping->ptr + mapping->size > mapping->reservation_ptr + mapping->reservation_size) {
      packet_error(response, "peer VA 0x%llx is not detached/reserved", (unsigned long long)mapping->ptr);
      return -1;
    }
  }
  for (handle = handles; handle != NULL; handle = handle->next) {
...
static int
handle_export(const struct dyn_vmm_packet* request, struct dyn_vmm_packet* response, int* export_fd)
{
  typedef CUresult(CUDAAPI * function_type)(
      void*, CUmemGenericAllocationHandle, CUmemAllocationHandleType, unsigned long long);
  function_type function = (function_type)resolve_next("cuMemExportToShareableHandle");
  retain_type retain_function = (retain_type)resolve_next("cuMemRetainAllocationHandle");
  release_type release_function = (release_type)resolve_next("cuMemRelease");
  struct owner_allocation* owner;
  struct context_scope scope;
  ...
  if (request->generation != generation || (phase != DYN_VMM_PHASE_AUDITED && phase != DYN_VMM_PHASE_CONTEXTS_SYNCED &&
                                            phase != DYN_VMM_PHASE_LOCK_READY && phase != DYN_VMM_PHASE_REATTACHING)) {
    packet_error(response, "fresh export generation/state mismatch");
    return -1;
  }
  owner = find_owner_by_id(request->object_dev, request->object_ino);
  if (owner == NULL || function == NULL || enter_context(owner->context, &scope) != 0) {
  ...
  if (owner->application_released) {
    if (owner->mapped_ptr == 0 || retain_function == NULL ||
        retain_function(&real, (void*)(uintptr_t)owner->mapped_ptr) != CUDA_SUCCESS)
      failed = true;
    else
      temporary = true;
  }
  if (!failed && (function(&fd, real, CU_MEM_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR, 0) != CUDA_SUCCESS ||
                  fd_identity(fd, &response->fresh_dev, &response->fresh_ino) != 0))
    failed = true;
  if (temporary && (release_function == NULL || release_function(real) != CUDA_SUCCESS)) {
...
static int
cleanup_import_replay(struct imported_allocation* allocation, CUmemGenericAllocationHandle temporary)
{
  unmap_type unmap_function = (unmap_type)resolve_next("cuMemUnmap");
  release_type release_function = (release_type)resolve_next("cuMemRelease");
  struct imported_mapping* mapping;
  ...
  if (temporary != 0 && (release_function == NULL || release_function(temporary) != CUDA_SUCCESS))
    failed = true;
  if (failed) {
...
static enum import_replay_state
import_replay_state(struct imported_allocation* allocation)
{
  struct imported_mapping* mapping;
  struct logical_handle* handle;
...
static int
handle_import(const struct dyn_vmm_packet* request, struct dyn_vmm_packet* response, int received_fd)
{
  import_type import_function = (import_type)resolve_next("cuMemImportFromShareableHandle");
  release_type release_function = (release_type)resolve_next("cuMemRelease");
  map_type map_function = (map_type)resolve_next("cuMemMap");
  struct imported_allocation* allocation;
  struct imported_mapping* mapping;
  ...
  if (request->generation != generation || (phase != DYN_VMM_PHASE_LOCK_READY && phase != DYN_VMM_PHASE_REATTACHING) ||
      fd_identity(received_fd, &fresh_dev, &fresh_ino) != 0 || fresh_dev != request->fresh_dev ||
      fresh_ino != request->fresh_ino) {
    packet_error(response, "fresh import packet/FD mismatch");
    return -1;
  }
  allocation = find_import(request->object_dev, request->object_ino);
  if (allocation == NULL) {
  ...
  switch (import_replay_state(allocation)) {
    ...
    case IMPORT_REPLAY_INCONSISTENT:
      packet_error(response, "imported peer object is neither fully active nor detached");
      return -1;
  }
  phase = DYN_VMM_PHASE_REATTACHING;
  if (import_function(&primary, (void*)(uintptr_t)received_fd, CU_MEM_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR) !=
      CUDA_SUCCESS) {
  ...
  for (handle = handles; handle != NULL; handle = handle->next) {
    ...
    if (import_function(&imported, (void*)(uintptr_t)received_fd, CU_MEM_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR) !=
        CUDA_SUCCESS) {
  ...
  for (mapping = mappings; mapping != NULL; mapping = mapping->next) {
    if (mapping->allocation != allocation)
      continue;
    if (!mapping->detached || scoped_map(map_function, mapping, primary) != 0) {
      packet_error(response, "exact peer replay failed at 0x%llx", (unsigned long long)mapping->ptr);
      goto fail;
    }
    mapping->detached = false;
    if (restore_access(mapping) != 0) {
      packet_error(response, "peer access replay failed at 0x%llx", (unsigned long long)mapping->ptr);
      goto fail;
    }
    stats.peer_reattached++;
    log_message(
        "event=peer_reattach generation=%llu va=0x%llx size=%zu offset=%zu fresh_real=0x%llx",
        (unsigned long long)generation, (unsigned long long)mapping->ptr, mapping->size, mapping->offset,
        (unsigned long long)primary);
  }
  if (temporary != 0) {
    if (release_function == NULL || release_function(temporary) != CUDA_SUCCESS) {
...
static int
handle_verify_active(const struct dyn_vmm_packet* request, struct dyn_vmm_packet* response)
{
  struct imported_mapping* mapping;
  struct logical_handle* handle;

  if (request->generation != generation || (phase != DYN_VMM_PHASE_REATTACHING && phase != DYN_VMM_PHASE_LOCK_READY &&
  ...
  for (mapping = mappings; mapping != NULL; mapping = mapping->next) {
    if (mapping->detached || verify_access(mapping) != 0) {
      packet_error(response, "peer mapping/access not active at 0x%llx", (unsigned long long)mapping->ptr);
      return -1;
    }
  }
  for (handle = handles; handle != NULL; handle = handle->next) {
...
static int
handle_commit(const struct dyn_vmm_packet* request, struct dyn_vmm_packet* response)
{
  if (request->generation != generation || phase != DYN_VMM_PHASE_ACTIVE_VERIFIED) {
  ...
  revision++;
  log_message(
      "event=commit generation=%llu owner_exports=%llu peer_imports=%llu peer_detached=%llu peer_reattached=%llu "
      "multicast_attempts=%llu unsupported=0x%x",
      (unsigned long long)generation, (unsigned long long)stats.owner_exports, (unsigned long long)stats.peer_imports,
      (unsigned long long)stats.peer_detached, (unsigned long long)stats.peer_reattached,
      (unsigned long long)stats.multicast_attempts, unsupported);
  return 0;
}

static int
handle_abort(const struct dyn_vmm_packet* request, struct dyn_vmm_packet* response)
{
  if (phase == DYN_VMM_PHASE_ACTIVE && generation == 0)
    return 0;
  if (request->generation != generation || phase == DYN_VMM_PHASE_POISONED) {
    packet_error(response, "abort generation/state mismatch");
    return -1;
  }
  if (phase <= DYN_VMM_PHASE_CONTEXTS_SYNCED || phase == DYN_VMM_PHASE_ACTIVE_VERIFIED) {
    phase = DYN_VMM_PHASE_ACTIVE;
    generation = 0;
    revision++;
    return 0;
  }
  packet_error(response, "detached peers require Snapshot-brokered best-effort remap");
  return -1;
}
...
serve_client(int client)
{
  ...
  if (receive_packet(client, &request, &received_fd) != 0 || request.magic != DYN_VMM_MAGIC ||
      request.version != DYN_VMM_VERSION || request.pid != (uint32_t)getpid() ||
      (request.operation == DYN_VMM_OP_IMPORT ? received_fd < 0 : received_fd >= 0)) {
  ...
  switch (request.operation) {
    ...
    case DYN_VMM_OP_IMPORT:
      (void)handle_import(&request, &response, received_fd);
      break;
    ...
    case DYN_VMM_OP_ABORT:
      ...
      break;
    default:
      packet_error(&response, "unknown control operation %u", request.operation);
      break;
  }
  set_packet_state(&response);
  pthread_mutex_unlock(&state_lock);
  (void)send_packet(client, &response, response.status == 0 ? export_fd : -1);
  if (export_fd >= 0)
    close(export_fd);
  if (received_fd >= 0)
    close(received_fd);
...
agent_main(void* unused)
{
  ...
  if (snprintf(path, sizeof(path), "%s/%s%d.sock", control_dir, DYN_VMM_SOCKET_PREFIX, getpid()) >= (int)sizeof(path)) {
  ...
  if (bind(fd, (struct sockaddr*)&address, sizeof(address)) != 0 || chmod(path, 0600) != 0 || listen(fd, 8) != 0) {
    close(fd);
    (void)unlink(path);
    pthread_mutex_unlock(&state_lock);
    return NULL;
...
start_agent(void)
{
  ...
  if (pthread_create(&thread, NULL, agent_main, NULL) != 0) {
    pthread_mutex_lock(&state_lock);
    mark_unsupported(DYN_VMM_UNSUPPORTED_BOOKKEEPING, "private Snapshot control thread failed to start");
    pthread_mutex_unlock(&state_lock);
    return;
  }
...
static void
observe_cuda_call(void)
{
  if (!enabled)
    return;
  ensure_agent_started();
  atomic_store_explicit(&cuda_observed, true, memory_order_relaxed);
}
...
{
  if (fork_child) {
    mark_unsupported(DYN_VMM_UNSUPPORTED_FORK, "CUDA VMM call after stateful fork");
    return true;
  }
  return false;
}
...
atfork_child(void)
{
  ...
  if (atomic_load_explicit(&cuda_observed, memory_order_relaxed) || owners != NULL || imports != NULL ||
      mappings != NULL) {
    fork_child = true;
    mark_unsupported(DYN_VMM_UNSUPPORTED_FORK, "CUDA VMM state inherited across fork");
  }
  pthread_mutex_unlock(&state_lock);
}
...
initialize(void)
{
  ...
  if (!enabled)
    return;
  force_posix = env_enabled("DYN_SNAPSHOT_CUDA_VMM_FORCE_POSIX");
  if (pthread_atfork(atfork_prepare, atfork_parent, atfork_child) != 0)
    mark_unsupported(DYN_VMM_UNSUPPORTED_FORK, "could not install fork bookkeeping");
}
...
CUresult CUDAAPI
cuMemAddressReserve(CUdeviceptr* ptr, size_t size, size_t alignment, CUdeviceptr address, unsigned long long flags)
{
  typedef CUresult(CUDAAPI * function_type)(CUdeviceptr*, size_t, size_t, CUdeviceptr, unsigned long long);
  function_type function = (function_type)resolve_next("cuMemAddressReserve");
  struct reservation* reservation;
  CUresult result;

  if (!enabled)
    return function != NULL ? function(ptr, size, alignment, address, flags) : unavailable();
  BEGIN_MANAGED_WRAPPER();
  if (reject_fork()) {
    END_MANAGED_WRAPPER();
    return CUDA_ERROR_NOT_SUPPORTED;
  }
  result = function != NULL ? function(ptr, size, alignment, address, flags) : unavailable();
  if (result == CUDA_SUCCESS) {
    reservation = calloc(1, sizeof(*reservation));
    if (reservation == NULL) {
      mark_unsupported(DYN_VMM_UNSUPPORTED_BOOKKEEPING, "VA reservation could not be recorded");
    } else {
      reservation->ptr = *ptr;
...
cuMemAddressFree(CUdeviceptr ptr, size_t size)
{
  ...
  function_type function = (function_type)resolve_next("cuMemAddressFree");
  ...
  for (mapping = mappings; mapping != NULL; mapping = mapping->next) {
    if (mapping->reservation_ptr == ptr && mapping->reservation_size == size) {
      END_MANAGED_WRAPPER();
      return CUDA_ERROR_INVALID_VALUE;
    }
  }
    ...
    CUmemGenericAllocationHandle* output, size_t size, const CUmemAllocationProp* properties, unsigned long long flags)
{
  ...
  function_type function = (function_type)resolve_next("cuMemCreate");
  release_type release_function = (release_type)resolve_next("cuMemRelease");
  struct owner_allocation* owner;
  CUresult result;
  bool fabric = properties != NULL && (properties->requestedHandleTypes & CU_MEM_HANDLE_TYPE_FABRIC) != 0;
  ...
  if (fabric && force_posix) {
    log_message("event=fabric_create_rejected reason=force_posix");
    END_MANAGED_WRAPPER();
    return CUDA_ERROR_NOT_SUPPORTED;
  }
  result = function != NULL ? function(output, size, properties, flags) : unavailable();
  if (result == CUDA_SUCCESS && is_synthetic(*output)) {
    if (release_function == NULL || release_function(*output) != CUDA_SUCCESS)
      poison("native owner token collision cleanup failed");
    else
      mark_unsupported(DYN_VMM_UNSUPPORTED_BOOKKEEPING, "native owner handle collided with logical token namespace");
    END_MANAGED_WRAPPER();
    return CUDA_ERROR_NOT_SUPPORTED;
  }
  if (result == CUDA_SUCCESS) {
    owner = calloc(1, sizeof(*owner));
    if (owner == NULL || current_context(&owner->context) != 0) {
      free(owner);
      mark_unsupported(DYN_VMM_UNSUPPORTED_BOOKKEEPING, "native owner metadata could not be recorded");
    } else {
      owner->application_real = *output;
      owner->next = owners;
      owners = owner;
      if (fabric)
        mark_unsupported(DYN_VMM_UNSUPPORTED_FABRIC, "live FABRIC VMM allocation is outside POSIX PoC scope");
      revision++;
    }
  }
  END_MANAGED_WRAPPER();
...
CUresult CUDAAPI
cuMemRelease(CUmemGenericAllocationHandle logical)
{
  release_type function = (release_type)resolve_next("cuMemRelease");
  ...
  if (reject_fork()) {
    END_MANAGED_WRAPPER();
    return is_synthetic(logical) ? CUDA_ERROR_INVALID_HANDLE : CUDA_ERROR_NOT_SUPPORTED;
  }
  if (!is_synthetic(logical)) {
...
CUresult CUDAAPI
cuMemMap(CUdeviceptr ptr, size_t size, size_t offset, CUmemGenericAllocationHandle logical, unsigned long long flags)
{
  ...
  if (!enabled)
    return function != NULL ? function(ptr, size, offset, logical, flags) : unavailable();
  BEGIN_MANAGED_WRAPPER();
  if (reject_fork()) {
    END_MANAGED_WRAPPER();
    return CUDA_ERROR_NOT_SUPPORTED;
  ...
  if (allocation == NULL) {
    result = function != NULL ? function(ptr, size, offset, real, flags) : unavailable();
    if (result == CUDA_SUCCESS) {
      ...
      if (owner != NULL) {
        ...
        if (current_context(&owner->context) != 0)
          mark_unsupported(DYN_VMM_UNSUPPORTED_BOOKKEEPING, "native owner map context unavailable");
      }
    }
    END_MANAGED_WRAPPER();
    return result;
  ...
  if (reservation == NULL) {
    mark_unsupported(DYN_VMM_UNSUPPORTED_UNKNOWN_HANDLE, "peer map does not belong to a tracked VA reservation");
    END_MANAGED_WRAPPER();
    return CUDA_ERROR_INVALID_VALUE;
  }
  mapping = calloc(1, sizeof(*mapping));
  if (mapping == NULL || current_context(&mapping->context) != 0) {
    free(mapping);
    END_MANAGED_WRAPPER();
...
{
  unmap_type function = (unmap_type)resolve_next("cuMemUnmap");
  ...
  if (result == CUDA_SUCCESS) {
    ...
    while (*cursor != NULL) {
      if ((*cursor)->ptr == ptr && (*cursor)->size == size) {
        ...
        struct imported_allocation* allocation = removed->allocation;
        *cursor = removed->next;
    ...
    for (owner = owners; owner != NULL; owner = owner->next) {
      if (owner->mapped_ptr == ptr && owner->mapped_size == size) {
        owner->mapped_ptr = 0;
        owner->mapped_size = 0;
        maybe_remove_owner(owner);
        break;
...
CUresult CUDAAPI
cuMemSetAccess(CUdeviceptr ptr, size_t size, const CUmemAccessDesc* descriptors, size_t count)
{
  set_access_type function = (set_access_type)resolve_next("cuMemSetAccess");
  struct pending {
    struct imported_mapping* mapping;
  ...
  if (!enabled)
    return function != NULL ? function(ptr, size, descriptors, count) : unavailable();
  BEGIN_MANAGED_WRAPPER();
  if (reject_fork()) {
    END_MANAGED_WRAPPER();
    return CUDA_ERROR_NOT_SUPPORTED;
  }
  if (size == 0 || ptr > UINT64_MAX - size || (count != 0 && descriptors == NULL)) {
    END_MANAGED_WRAPPER();
    return CUDA_ERROR_INVALID_VALUE;
  }
  end = ptr + size;
  for (mapping = mappings; mapping != NULL; mapping = mapping->next) {
    CUdeviceptr start = ptr > mapping->ptr ? ptr : mapping->ptr;
    CUdeviceptr mapping_end = mapping->ptr + mapping->size;
    CUdeviceptr intersection_end = end < mapping_end ? end : mapping_end;
    ...
    if (count != 0) {
      update->record->descriptors = calloc(count, sizeof(*descriptors));
      ...
      memcpy(update->record->descriptors, descriptors, count * sizeof(*descriptors));
    }
  ...
  result = function != NULL ? function(ptr, size, descriptors, count) : unavailable();
  if (result == CUDA_SUCCESS) {
    while (pending != NULL) {
      struct pending* update = pending;
      struct access_record** access_tail = &update->mapping->access;
      while (*access_tail != NULL) access_tail = &(*access_tail)->next;
...
cuMemGetAccess(unsigned long long* flags, const CUmemLocation* location, CUdeviceptr ptr)
{
  typedef CUresult(CUDAAPI * function_type)(unsigned long long*, const CUmemLocation*, CUdeviceptr);
  function_type function = (function_type)resolve_next("cuMemGetAccess");
  return function != NULL ? function(flags, location, ptr) : unavailable();
}

CUresult CUDAAPI
cuMemExportToShareableHandle(
    void* shareable, CUmemGenericAllocationHandle logical, CUmemAllocationHandleType handle_type,
    unsigned long long flags)
{
  ...
  if (!enabled)
    return function != NULL ? function(shareable, logical, handle_type, flags) : unavailable();
  BEGIN_MANAGED_WRAPPER();
  if (reject_fork()) {
    END_MANAGED_WRAPPER();
    return CUDA_ERROR_NOT_SUPPORTED;
  ...
  if (handle_type != CU_MEM_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR) {
    log_message("event=non_posix_export_rejected handle_type=0x%x", (unsigned int)handle_type);
    END_MANAGED_WRAPPER();
    return CUDA_ERROR_NOT_SUPPORTED;
  }
  if (is_synthetic(logical)) {
    mark_unsupported(DYN_VMM_UNSUPPORTED_UNKNOWN_HANDLE, "re-export of an imported peer is unsupported");
    END_MANAGED_WRAPPER();
    return CUDA_ERROR_NOT_SUPPORTED;
  }
  real = logical;
  result = function != NULL ? function(shareable, real, handle_type, flags) : unavailable();
  if (result == CUDA_SUCCESS) {
    owner = find_owner(real);
    if (owner == NULL || fd_identity(*(int*)shareable, &dev, &ino) != 0) {
      mark_unsupported(DYN_VMM_UNSUPPORTED_UNKNOWN_HANDLE, "POSIX export did not match a native owner");
    } else if (owner->exported && (owner->object_dev != dev || owner->object_ino != ino)) {
      mark_unsupported(DYN_VMM_UNSUPPORTED_UNKNOWN_HANDLE, "native owner export identity changed");
    } else {
      owner->exported = true;
      owner->object_dev = dev;
      owner->object_ino = ino;
      if (current_context(&owner->context) != 0)
        mark_unsupported(DYN_VMM_UNSUPPORTED_BOOKKEEPING, "owner export context unavailable");
      stats.owner_exports++;
      log_message(
          "event=owner_export object=%llu:%llu real=0x%llx", (unsigned long long)dev, (unsigned long long)ino,
    ...
    CUmemGenericAllocationHandle* output, void* os_handle, CUmemAllocationHandleType handle_type)
{
  ...
  if (!enabled)
    return function != NULL ? function(output, os_handle, handle_type) : unavailable();
  BEGIN_MANAGED_WRAPPER();
  if (reject_fork()) {
    END_MANAGED_WRAPPER();
    return CUDA_ERROR_NOT_SUPPORTED;
  ...
  if (handle_type != CU_MEM_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR) {
    log_message("event=non_posix_import_rejected handle_type=0x%x", (unsigned int)handle_type);
    END_MANAGED_WRAPPER();
    return CUDA_ERROR_NOT_SUPPORTED;
  }
  if (fd_identity((int)(uintptr_t)os_handle, &dev, &ino) != 0) {
    END_MANAGED_WRAPPER();
    return CUDA_ERROR_INVALID_HANDLE;
  }
  ...
  result = function != NULL ? function(&real, os_handle, handle_type) : unavailable();
  if (result != CUDA_SUCCESS) {
    free(new_allocation);
    free(handle);
    END_MANAGED_WRAPPER();
...
cuMemGetAllocationGranularity(
    size_t* granularity, const CUmemAllocationProp* properties, CUmemAllocationGranularity_flags option)
{
  typedef CUresult(CUDAAPI * function_type)(size_t*, const CUmemAllocationProp*, CUmemAllocationGranularity_flags);
  function_type function = (function_type)resolve_next("cuMemGetAllocationGranularity");

  return function != NULL ? function(granularity, properties, option) : unavailable();
}

CUresult CUDAAPI
cuMemGetAllocationPropertiesFromHandle(CUmemAllocationProp* properties, CUmemGenericAllocationHandle logical)
{
  typedef CUresult(CUDAAPI * function_type)(CUmemAllocationProp*, CUmemGenericAllocationHandle);
  function_type function = (function_type)resolve_next("cuMemGetAllocationPropertiesFromHandle");
  CUmemGenericAllocationHandle real;
  CUresult result;

  if (!enabled)
    return function != NULL ? function(properties, logical) : unavailable();
  BEGIN_MANAGED_WRAPPER();
  result = translate_import(logical, &real, NULL);
  if (result == CUDA_SUCCESS)
    result = function != NULL ? function(properties, real) : unavailable();
  END_MANAGED_WRAPPER();
  return result;
}
...
cuMemRetainAllocationHandle(CUmemGenericAllocationHandle* output, void* address)
{
  ...
  function_type function = (function_type)resolve_next("cuMemRetainAllocationHandle");
  struct imported_mapping* mapping;
  struct logical_handle* handle;
  CUmemGenericAllocationHandle real;
  CUresult result;
  ...
  mapping = find_mapping_covering((CUdeviceptr)(uintptr_t)address);
  if (mapping == NULL) {
  ...
  result = function != NULL ? function(&real, address) : unavailable();
  if (result != CUDA_SUCCESS) {
    free(handle);
    END_MANAGED_WRAPPER();
    return result;
...
mem_map_array_async(const char* real_symbol, CUarrayMapInfo* map_info, unsigned int count, CUstream stream)
{
  ...
  if (enabled) {
    for (index = 0; index < count; index++) {
      if (map_info[index].memOperationType == CU_MEM_OPERATION_TYPE_MAP &&
          map_info[index].memHandleType == CU_MEM_HANDLE_TYPE_GENERIC &&
          is_synthetic(map_info[index].memHandle.memHandle)) {
        observe_cuda_call();
        pthread_mutex_lock(&state_lock);
        mark_unsupported(DYN_VMM_UNSUPPORTED_UNKNOWN_HANDLE, "array mapping of imported peers is unsupported");
        pthread_mutex_unlock(&state_lock);
        return CUDA_ERROR_NOT_SUPPORTED;
      }
    }
...
static CUresult
legacy_ipc_result(CUresult result)
{
  if (enabled) {
    observe_cuda_call();
    pthread_mutex_lock(&state_lock);
    mark_unsupported(DYN_VMM_UNSUPPORTED_LEGACY_IPC, "legacy CUDA IPC observed");
    pthread_mutex_unlock(&state_lock);
  }
  return result;
}
...
{
  typedef CUresult(CUDAAPI * function_type)(CUipcMemHandle*, CUdeviceptr);
  function_type function = (function_type)resolve_next("cuIpcGetMemHandle");
  return legacy_ipc_result(function != NULL ? function(handle, ptr) : unavailable());
}
...
cuIpcOpenMemHandle_v2(CUdeviceptr* ptr, CUipcMemHandle handle, unsigned int flags)
{
  ...
  function_type function = (function_type)resolve_next("cuIpcOpenMemHandle_v2");
...
cuIpcCloseMemHandle(CUdeviceptr ptr)
{
  ...
  function_type function = (function_type)resolve_next("cuIpcCloseMemHandle");
  return legacy_ipc_result(function != NULL ? function(ptr) : unavailable());
}

static CUresult
reject_multicast(const char* operation)
{
  observe_cuda_call();
  pthread_mutex_lock(&state_lock);
  stats.multicast_attempts++;
  pthread_mutex_unlock(&state_lock);
  log_message("event=multicast_rejected operation=%s", operation);
  return CUDA_ERROR_NOT_SUPPORTED;
}
...
cuMulticastCreate(CUmemGenericAllocationHandle* output, const CUmulticastObjectProp* properties)
{
  ...
  function_type function = (function_type)resolve_next("cuMulticastCreate");
...
CUresult CUDAAPI
cuMulticastAddDevice(CUmemGenericAllocationHandle handle, CUdevice device)
{
  typedef CUresult(CUDAAPI * function_type)(CUmemGenericAllocationHandle, CUdevice);
  function_type function = (function_type)resolve_next("cuMulticastAddDevice");
    ...
    size_t memory_offset, size_t size, unsigned long long flags)
{
  ...
  function_type function = (function_type)resolve_next("cuMulticastBindMem");
    ...
    CUmemGenericAllocationHandle memory, size_t memory_offset, size_t size, unsigned long long flags)
{
  ...
  function_type function = (function_type)resolve_next("cuMulticastBindMem_v2");
...
CUresult CUDAAPI
cuMulticastBindAddr(
    CUmemGenericAllocationHandle multicast, size_t multicast_offset, CUdeviceptr ptr, size_t size,
    unsigned long long flags)
{
  typedef CUresult(CUDAAPI * function_type)(
      CUmemGenericAllocationHandle, size_t, CUdeviceptr, size_t, unsigned long long);
  function_type function = (function_type)resolve_next("cuMulticastBindAddr");

  if (!enabled)
    return function != NULL ? function(multicast, multicast_offset, ptr, size, flags) : unavailable();
  return reject_multicast("cuMulticastBindAddr");
}

CUresult CUDAAPI
cuMulticastBindAddr_v2(
    CUmemGenericAllocationHandle multicast, CUdevice device, size_t multicast_offset, CUdeviceptr ptr, size_t size,
    unsigned long long flags)
{
  typedef CUresult(CUDAAPI * function_type)(
      CUmemGenericAllocationHandle, CUdevice, size_t, CUdeviceptr, size_t, unsigned long long);
  function_type function = (function_type)resolve_next("cuMulticastBindAddr_v2");

  if (!enabled)
    return function != NULL ? function(multicast, device, multicast_offset, ptr, size, flags) : unavailable();
  return reject_multicast("cuMulticastBindAddr_v2");
}

CUresult CUDAAPI
cuMulticastUnbind(CUmemGenericAllocationHandle multicast, CUdevice device, size_t multicast_offset, size_t size)
{
  typedef CUresult(CUDAAPI * function_type)(CUmemGenericAllocationHandle, CUdevice, size_t, size_t);
  function_type function = (function_type)resolve_next("cuMulticastUnbind");

  if (!enabled)
    return function != NULL ? function(multicast, device, multicast_offset, size) : unavailable();
    ...
    size_t* granularity, const CUmulticastObjectProp* properties, CUmulticastGranularity_flags option)
{
  ...
  function_type function = (function_type)resolve_next("cuMulticastGetGranularity");
...
static void
mark_resolver_abi_unsupported(void)
{
  if (!enabled)
    return;
  pthread_mutex_lock(&state_lock);
  mark_unsupported(DYN_VMM_UNSUPPORTED_RESOLVER_ABI, "managed symbol requested with unsupported CUDA ABI version");
  pthread_mutex_unlock(&state_lock);
}
...
CUresult CUDAAPI
cuGetProcAddress(const char* symbol, void** function_pointer, int cuda_version, cuuint64_t flags)
{
  typedef CUresult(CUDAAPI * function_type)(const char*, void**, int, cuuint64_t);
  function_type function = (function_type)resolve_next("cuGetProcAddress");
  CUresult result = function != NULL ? function(symbol, function_pointer, cuda_version, flags) : unavailable();
  void* replacement;
    ...
    CUdriverProcAddressQueryResult* status)
{
  ...
  function_type function = (function_type)resolve_next("cuGetProcAddress_v2");
  CUresult result = function != NULL ? function(symbol, function_pointer, cuda_version, flags, status) : unavailable();
  void* replacement;
...
cudaGetDriverEntryPointByVersion(
    const char* symbol, void** function_pointer, unsigned int cuda_version, unsigned long long flags,
