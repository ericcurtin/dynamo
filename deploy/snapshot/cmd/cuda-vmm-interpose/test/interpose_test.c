...
static bool synthetic(CUmemGenericAllocationHandle handle)
{
    return (handle & SYNTHETIC_MASK) == SYNTHETIC_TAG;
}

typedef CUresult(CUDAAPI * multicast_create_type)(
    CUmemGenericAllocationHandle *,
    const CUmulticastObjectProp *);
typedef CUresult(CUDAAPI * multicast_add_device_type)(
...
typedef CUresult(CUDAAPI * multicast_unbind_type)(
...
static void require_multicast_call(
    unsigned int operation, const uint64_t arguments[7],
    const char *message)
{
    unsigned int index;

    require(
        fake_cuda_multicast_operation() == operation,
        message);
    for (index = 0; index < 7; index++)
        require(
            fake_cuda_multicast_argument(index) ==
                arguments[index],
            message);
}

static void exercise_dormant_multicast(
    const struct multicast_functions *functions)
{
    ...
    require(
        functions->create(&output, &properties) ==
                CUDA_ERROR_UNKNOWN &&
    ...
    require_multicast_call(
        1, arguments, "dormant multicast create arguments changed");

    require(
        functions->add_device(UINT64_C(0x11), 2) ==
            CUDA_ERROR_UNKNOWN,
        "dormant multicast add-device result changed");
    memset(arguments, 0, sizeof(arguments));
    arguments[0] = UINT64_C(0x11);
    arguments[1] = 2;
    require_multicast_call(
        2, arguments,
        "dormant multicast add-device arguments changed");

    require(
        functions->bind_mem(
            UINT64_C(0x12), 0x13, UINT64_C(0x14),
            0x15, 0x16, UINT64_C(0x17)) ==
            CUDA_ERROR_UNKNOWN,
        "dormant multicast bind-mem result changed");
    {
        const uint64_t expected[7] = {
            UINT64_C(0x12), UINT64_C(0x13),
            UINT64_C(0x14), UINT64_C(0x15),
            UINT64_C(0x16), UINT64_C(0x17), 0,
        };
        require_multicast_call(
            3, expected,
            "dormant multicast bind-mem arguments changed");
    }

    require(
        functions->bind_mem_v2(
            UINT64_C(0x21), 3, 0x22,
            UINT64_C(0x23), 0x24, 0x25,
            UINT64_C(0x26)) == CUDA_ERROR_UNKNOWN,
        "dormant multicast bind-mem-v2 result changed");
    {
        const uint64_t expected[7] = {
            UINT64_C(0x21), 3, UINT64_C(0x22),
            UINT64_C(0x23), UINT64_C(0x24),
            UINT64_C(0x25), UINT64_C(0x26),
        };
        require_multicast_call(
            4, expected,
            "dormant multicast bind-mem-v2 arguments changed");
    }

    require(
        functions->bind_addr(
            UINT64_C(0x31), 0x32, UINT64_C(0x33),
            0x34, UINT64_C(0x35)) == CUDA_ERROR_UNKNOWN,
        "dormant multicast bind-addr result changed");
    {
        const uint64_t expected[7] = {
            UINT64_C(0x31), UINT64_C(0x32),
            UINT64_C(0x33), UINT64_C(0x34),
            UINT64_C(0x35), 0, 0,
        };
        require_multicast_call(
            5, expected,
            "dormant multicast bind-addr arguments changed");
    }

    require(
        functions->bind_addr_v2(
            UINT64_C(0x41), 4, 0x42,
            UINT64_C(0x43), 0x44,
            UINT64_C(0x45)) == CUDA_ERROR_UNKNOWN,
        "dormant multicast bind-addr-v2 result changed");
    {
        const uint64_t expected[7] = {
            UINT64_C(0x41), 4, UINT64_C(0x42),
            UINT64_C(0x43), UINT64_C(0x44),
            UINT64_C(0x45), 0,
        };
        require_multicast_call(
            6, expected,
            "dormant multicast bind-addr-v2 arguments changed");
    }

    require(
        functions->unbind(
            UINT64_C(0x51), 5, 0x52, 0x53) ==
            CUDA_ERROR_UNKNOWN,
        "dormant multicast unbind result changed");
    {
        const uint64_t expected[7] = {
            UINT64_C(0x51), 5, UINT64_C(0x52),
            UINT64_C(0x53), 0, 0, 0,
        };
        require_multicast_call(
            7, expected,
            "dormant multicast unbind arguments changed");
    }
    ...
    require_multicast_call(
        8, arguments,
        "dormant multicast granularity arguments changed");
    require(
        fake_cuda_multicast_calls() == initial_calls + 8,
        "dormant multicast call count changed");
}
...
static int send_packet(
    int socket_fd, const struct dyn_vmm_packet *packet, int passed_fd)
{
    struct iovec iov = {
...
static int receive_packet(
    int socket_fd, struct dyn_vmm_packet *packet, int *received_fd)
{
    struct iovec iov = {
        .iov_base = packet,
        .iov_len = sizeof(*packet),
    ...
    *received_fd = -1;
    ...
    if (header != NULL) {
        if (header->cmsg_level != SOL_SOCKET ||
            ...
            return -1;
        memcpy(
            received_fd, CMSG_DATA(header),
            sizeof(*received_fd));
    }
    return 0;
}

static struct dyn_vmm_packet control(
    uint16_t operation, uint64_t generation,
    const struct dyn_vmm_packet *fields,
    int passed_fd, int *received_fd,
    bool success)
{
    ...
    require(
        send_packet(socket_fd, &request, passed_fd) == 0,
        "send control packet");
    require(
        receive_packet(
            socket_fd, &response,
            received_fd != NULL ? received_fd : &ignored) == 0,
    ...
    require(
        ...
        success ? response.message
                : "control operation unexpectedly succeeded");
    return response;
}

static size_t audit(
    uint64_t generation,
    struct dyn_vmm_packet *records,
    size_t capacity,
    struct dyn_vmm_packet *summary)
{
    ...
    require(
        receive_packet(socket_fd, summary, &received_fd) == 0 &&
            received_fd < 0 && summary->status == 0,
        "receive audit summary");
    require(summary->count <= capacity, "audit capacity exceeded");
    for (index = 0; index < summary->count; index++) {
        require(
            receive_packet(
                socket_fd, &records[index], &received_fd) == 0 &&
                received_fd < 0 &&
...
static void require_audit_failure(uint64_t generation)
{
    struct dyn_vmm_packet request = {
        .magic = DYN_VMM_MAGIC,
        .version = DYN_VMM_VERSION,
    ...
    require(send_packet(socket_fd, &request, -1) == 0, "send audit");
    require(
        receive_packet(socket_fd, &response, &received_fd) == 0 &&
            received_fd < 0 && response.status != 0,
        "unsupported live resource did not fail audit");
    close(socket_fd);
}

static void test_dormant(void)
{
    ...
    require(
        cuGetProcAddress(
            "cuMemCreate", &entry, CUDA_VERSION, 0) ==
            CUDA_SUCCESS,
        "dormant resolver call");
    require(
        entry == fake_cuda_last_resolved_entry(),
        "dormant resolver substituted a wrapper");
    require(
        cuMemCreate(&handle, 4096, &properties, 0) ==
            CUDA_SUCCESS &&
            !synthetic(handle),
...
#define RESOLVE_MULTICAST(field, type, name)                                  \
    do {                                                                       \
        ...
        require(                                                              \
            cuGetProcAddress(                                                  \
                #name, &entry, 13010, 0) == CUDA_SUCCESS &&                    \
                entry == fake_cuda_last_resolved_entry(),                      \
            "dormant resolver substituted multicast " #name);                 \
        resolver_functions.field = (type)entry;                               \
    } while (0)
    RESOLVE_MULTICAST(
        create, multicast_create_type, cuMulticastCreate);
    RESOLVE_MULTICAST(
        add_device, multicast_add_device_type,
        cuMulticastAddDevice);
    RESOLVE_MULTICAST(
        bind_mem, multicast_bind_mem_type, cuMulticastBindMem);
    RESOLVE_MULTICAST(
        bind_mem_v2, multicast_bind_mem_v2_type,
        cuMulticastBindMem_v2);
    RESOLVE_MULTICAST(
        bind_addr, multicast_bind_addr_type,
        cuMulticastBindAddr);
    RESOLVE_MULTICAST(
        bind_addr_v2, multicast_bind_addr_v2_type,
        cuMulticastBindAddr_v2);
    RESOLVE_MULTICAST(
        unbind, multicast_unbind_type, cuMulticastUnbind);
    RESOLVE_MULTICAST(
        granularity, multicast_granularity_type,
        cuMulticastGetGranularity);
#undef RESOLVE_MULTICAST
    exercise_dormant_multicast(&resolver_functions);
}

static void test_resolvers(void)
{
    CUmemAllocationProp properties = {0};
    CUmemGenericAllocationHandle handle;
    ...
    require(
        cudaGetDriverEntryPointByVersion(
            "cuMemRetainAllocationHandle", &resolved,
            12000, 0, &runtime_status) == cudaSuccess &&
...
static void test_peer_lifecycle(bool reset)
{
    ...
    require(
        cuMemAddressReserve(
            &shared_reservation, 12288, 0, 0, 0) ==
                CUDA_SUCCESS,
        "shared owner/peer reservation");
    owner_va = shared_reservation;
    peer_va = shared_reservation + 8192;
    require(
            cuMemCreate(&owner, 8192, &properties, 0) ==
                CUDA_SUCCESS &&
            !synthetic(owner) &&
            cuMemMap(owner_va, 8192, 0, owner, 0) ==
                CUDA_SUCCESS &&
    ...
    require(
        cuMemImportFromShareableHandle(
            &imported, (void *)(uintptr_t)original_fd,
            CU_MEM_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR) ==
                CUDA_SUCCESS &&
            synthetic(imported),
        "peer import did not return a synthetic token");
    require(
        cuMemMap(peer_va, 4096, 4096, imported, 0) ==
                CUDA_SUCCESS &&
            cuMemSetAccess(owner_va, 12288, &peer_access, 1) ==
                CUDA_SUCCESS,
        "mixed owner/peer access setup");
    require(
        cuMemGetAllocationPropertiesFromHandle(
            &imported_properties, imported) == CUDA_SUCCESS &&
            cuMemRetainAllocationHandle(
                &retained, (void *)(uintptr_t)peer_va) ==
                CUDA_SUCCESS &&
            synthetic(retained),
    ...
    require(
        cuMemRelease(
            SYNTHETIC_TAG | UINT64_C(0x1234)) ==
            CUDA_ERROR_INVALID_HANDLE,
        "unknown synthetic token reached fake CUDA");
    require(
        audit(55, records, 4, &summary) == 2 &&
            summary.phase == DYN_VMM_PHASE_AUDITED,
    ...
    require(
        owner_record.object_dev != 0 &&
            owner_record.object_ino != 0,
        "owner identity missing");
    (void)control(
        DYN_VMM_OP_SYNC_ALL_CONTEXTS, 55,
        NULL, -1, NULL, true);
    require(
        fake_cuda_sync_calls() >= 1,
        "participating contexts were not synchronized");
    (void)control(
        DYN_VMM_OP_DETACH_IMPORTS, 55,
        NULL, -1, NULL, true);
    (void)control(
        DYN_VMM_OP_VERIFY_LOCK_READY, 55,
        NULL, -1, NULL, true);
    require(
        fake_cuda_live_maps() == 1 &&
            fake_cuda_live_reservations() == 1 &&
            fake_cuda_live_objects() == 1 &&
            fake_cuda_live_handles() == 0 &&
            memcmp(
                (void *)(uintptr_t)owner_va,
                pattern, sizeof(pattern)) == 0,
        "detach changed native owner or peer VA reservation");
    require(
        cuMemRelease(imported) == CUDA_ERROR_INVALID_HANDLE,
        "detached synthetic token reached fake CUDA");

    export_response = control(
        DYN_VMM_OP_EXPORT, 55, &owner_record,
        -1, &fresh_fd, true);
    require(
        fresh_fd >= 0,
        "fresh owner export missing SCM_RIGHTS FD");
    (void)control(
        DYN_VMM_OP_IMPORT, 55, &export_response,
        fresh_fd, NULL, true);
    close(fresh_fd);
    (void)control(
        DYN_VMM_OP_VERIFY_ACTIVE, 55,
        NULL, -1, NULL, true);
    (void)control(
        DYN_VMM_OP_COMMIT, 55,
        NULL, -1, NULL, true);
    require(
        fake_cuda_live_maps() == 2 &&
            fake_cuda_live_reservations() == 1 &&
            fake_cuda_live_objects() == 1 &&
            fake_cuda_live_handles() == 2 &&
            cuMemGetAccess(
                &access, &peer_access.location, peer_va) ==
                CUDA_SUCCESS &&
            access == CU_MEM_ACCESS_FLAGS_PROT_READWRITE &&
            memcmp(
                (void *)(uintptr_t)owner_va,
                pattern, sizeof(pattern)) == 0 &&
            fake_cuda_import_calls() == 3,
        "fresh import did not restore exact VA/access/logical references");

    require(
        cuMemUnmap(peer_va, 4096) == CUDA_SUCCESS &&
            cuMemRelease(imported) == CUDA_SUCCESS &&
            cuMemRelease(retained) == CUDA_SUCCESS &&
            cuMemUnmap(owner_va, 8192) == CUDA_SUCCESS &&
            cuMemAddressFree(shared_reservation, 12288) == CUDA_SUCCESS,
        "lifecycle cleanup failed");
    require(
        fake_cuda_live_maps() == 0 &&
            fake_cuda_live_reservations() == 0 &&
            fake_cuda_live_handles() == 0 &&
            fake_cuda_live_objects() == 0,
        "lifecycle cleanup leaked native or imported CUDA state");
}

static void test_release_failure_rollback(void)
{
    CUmemAllocationProp properties = {
        .type = CU_MEM_ALLOCATION_TYPE_PINNED,
        .location = {
    ...
    require(
        cuMemAddressReserve(&reservation, 12288, 0, 0, 0) ==
            CUDA_SUCCESS,
    ...
    require(
        cuMemcpyHtoD_v2(
            owner_va, pattern, sizeof(pattern)) == CUDA_SUCCESS,
        "release rollback owner pattern initialization");
    require(
        cuMemImportFromShareableHandle(
            &imported, (void *)(uintptr_t)export_fd,
            CU_MEM_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR) ==
                CUDA_SUCCESS &&
            cuMemMap(peer_va, 4096, 4096, imported, 0) ==
                CUDA_SUCCESS &&
            cuMemSetAccess(peer_va, 4096, &peer_access, 1) ==
                CUDA_SUCCESS &&
            cuMemRetainAllocationHandle(
                &retained, (void *)(uintptr_t)peer_va) ==
                CUDA_SUCCESS,
    ...
    close(export_fd);
    require(
        audit(71, records, 4, &summary) == 2 &&
            summary.phase == DYN_VMM_PHASE_AUDITED,
        "release rollback audit");
    (void)control(
        DYN_VMM_OP_SYNC_ALL_CONTEXTS, 71,
        NULL, -1, NULL, true);
    fake_cuda_fail_after("release", 1);
    response = control(
        DYN_VMM_OP_DETACH_IMPORTS, 71,
        NULL, -1, NULL, false);
    require(
        response.phase == DYN_VMM_PHASE_CONTEXTS_SYNCED &&
            response.generation == 71 &&
            strstr(
                response.message,
                "local peer rollback restored active mappings") != NULL &&
            fake_cuda_live_objects() == 1 &&
            fake_cuda_live_maps() == 2 &&
            fake_cuda_live_handles() == 3 &&
            fake_cuda_live_references() == 3 &&
            fake_cuda_map_calls() == 3 &&
            fake_cuda_unmap_calls() == 1 &&
            cuMemGetAccess(
                &access, &peer_access.location, peer_va) ==
                CUDA_SUCCESS &&
            access == CU_MEM_ACCESS_FLAGS_PROT_READWRITE &&
    ...
    require(
        audit(71, records, 4, &summary) == 2 &&
            summary.generation == 71 &&
            summary.phase == DYN_VMM_PHASE_CONTEXTS_SYNCED,
        "release rollback state/object graph is not queryable");
    response = control(
        DYN_VMM_OP_ABORT, 71, NULL, -1, NULL, true);
    require(
        response.generation == 0 &&
            response.phase == DYN_VMM_PHASE_ACTIVE,
        "release rollback did not reset generation");
    require(
        cuMemUnmap(peer_va, 4096) == CUDA_SUCCESS &&
            cuMemRelease(imported) == CUDA_SUCCESS &&
            cuMemRelease(retained) == CUDA_SUCCESS &&
            cuMemUnmap(owner_va, 8192) == CUDA_SUCCESS &&
            cuMemRelease(owner) == CUDA_SUCCESS &&
            cuMemAddressFree(reservation, 12288) == CUDA_SUCCESS,
        "release rollback cleanup");
    require(
        fake_cuda_live_objects() == 0 &&
            fake_cuda_live_handles() == 0 &&
            fake_cuda_live_references() == 0 &&
            fake_cuda_live_maps() == 0 &&
            fake_cuda_live_reservations() == 0 &&
...
static void test_multi_object_replay_retry(void)
{
    CUmemAllocationProp properties = {
        .type = CU_MEM_ALLOCATION_TYPE_PINNED,
        .location = {
    ...
    for (index = 0; index < 2; index++) {
        require(
            cuMemAddressReserve(
                &reservations[index], 8192, 0, 0, 0) ==
                    CUDA_SUCCESS,
            "multi-object reservation");
        owner_vas[index] = reservations[index];
        peer_vas[index] = reservations[index] + 4096;
        require(
            cuMemCreate(
                &owners[index], 8192, &properties, 0) ==
                    CUDA_SUCCESS &&
                cuMemMap(
                    owner_vas[index], 4096, 0,
                    owners[index], 0) == CUDA_SUCCESS &&
                cuMemExportToShareableHandle(
                    &original_fds[index], owners[index],
                    CU_MEM_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR,
                    0) == CUDA_SUCCESS &&
                cuMemImportFromShareableHandle(
                    &imports[index],
                    (void *)(uintptr_t)original_fds[index],
                    CU_MEM_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR) ==
                    CUDA_SUCCESS &&
                cuMemMap(
                    peer_vas[index], 4096, 4096,
                    imports[index], 0) == CUDA_SUCCESS &&
                cuMemSetAccess(
                    peer_vas[index], 4096,
                    &peer_access, 1) == CUDA_SUCCESS,
            "multi-object fixture");
        require(
            synthetic(imports[index]),
            "multi-object import did not return stable synthetic handle");
        require(
            fstat(original_fds[index], &status) == 0,
            "multi-object original FD identity");
        owner_records[index].object_dev =
            (uint64_t)status.st_dev;
        owner_records[index].object_ino =
    ...
    require(
        cuMemRetainAllocationHandle(
            &retained, (void *)(uintptr_t)peer_vas[1]) ==
            CUDA_SUCCESS,
        "multi-object second logical reference");
    require(
        audit(73, records, 8, &summary) == 4,
        "multi-object audit");
    (void)control(
        DYN_VMM_OP_SYNC_ALL_CONTEXTS, 73,
        NULL, -1, NULL, true);
    (void)control(
        DYN_VMM_OP_DETACH_IMPORTS, 73,
        NULL, -1, NULL, true);
    (void)control(
        DYN_VMM_OP_VERIFY_LOCK_READY, 73,
        NULL, -1, NULL, true);

    for (index = 0; index < 2; index++) {
        exports[index] = control(
            DYN_VMM_OP_EXPORT, 73, &owner_records[index],
            -1, &fresh_fds[index], true);
        require(fresh_fds[index] >= 0, "multi-object fresh export");
    }
    (void)control(
        DYN_VMM_OP_IMPORT, 73, &exports[0],
        fresh_fds[0], NULL, true);
    fake_cuda_fail_after("import", 1);
    (void)control(
        DYN_VMM_OP_IMPORT, 73, &exports[1],
        fresh_fds[1], NULL, false);
    imports_before_retry = fake_cuda_import_calls();
    (void)control(
        DYN_VMM_OP_IMPORT, 73, &exports[0],
        fresh_fds[0], NULL, true);
    require(
        fake_cuda_import_calls() == imports_before_retry,
        "retry re-imported an already-active object");
    (void)control(
        DYN_VMM_OP_IMPORT, 73, &exports[1],
        fresh_fds[1], NULL, true);
    close(fresh_fds[0]);
    close(fresh_fds[1]);
    (void)control(
        DYN_VMM_OP_VERIFY_ACTIVE, 73,
        NULL, -1, NULL, true);
    response = control(
        DYN_VMM_OP_ABORT, 73,
        NULL, -1, NULL, true);
    require(
        response.generation == 0 &&
            response.phase == DYN_VMM_PHASE_ACTIVE &&
            fake_cuda_live_objects() == 2 &&
            fake_cuda_live_maps() == 4 &&
            fake_cuda_live_handles() == 5 &&
            fake_cuda_live_references() == 5 &&
    ...
    for (index = 0; index < 2; index++) {
        ...
        require(
            cuMemGetAccess(
                &access, &peer_access.location,
                peer_vas[index]) == CUDA_SUCCESS &&
                access == CU_MEM_ACCESS_FLAGS_PROT_READWRITE &&
                fake_cuda_map_offset(peer_vas[index]) == 4096 &&
                synthetic(imports[index]),
            "multi-object retry did not preserve handle/VA/offset/access");
    }
    require(
        cuMemUnmap(peer_vas[0], 4096) == CUDA_SUCCESS &&
            cuMemRelease(imports[0]) == CUDA_SUCCESS &&
            cuMemUnmap(peer_vas[1], 4096) == CUDA_SUCCESS &&
            cuMemRelease(imports[1]) == CUDA_SUCCESS &&
            cuMemRelease(retained) == CUDA_SUCCESS,
        "multi-object peer cleanup");
    for (index = 0; index < 2; index++) {
        require(
            cuMemUnmap(owner_vas[index], 4096) == CUDA_SUCCESS &&
                cuMemRelease(owners[index]) == CUDA_SUCCESS &&
                cuMemAddressFree(
                    reservations[index], 8192) == CUDA_SUCCESS,
...
static void test_force_posix_negotiation(void)
{
...
static void test_live_fabric_fails_closed(void)
{
    ...
    fake_cuda_reset();
    require(
        cuMemCreate(&handle, 4096, &fabric, 0) ==
                CUDA_SUCCESS &&
            fake_cuda_create_calls() == 1 &&
            fake_cuda_live_objects() == 1,
        "live FABRIC fixture was not created");
    require_audit_failure(91);
    require(
        cuMemRelease(handle) == CUDA_SUCCESS,
        "live FABRIC fixture cleanup failed");
}

static void test_unsupported(void)
{
    typedef CUresult(CUDAAPI * ipc_type)(
        CUipcMemHandle *, CUdeviceptr);
    typedef CUresult(CUDAAPI * multicast_type)(
        CUmemGenericAllocationHandle *,
        const CUmulticastObjectProp *);
    CUmemAllocationProp properties = {
        .type = CU_MEM_ALLOCATION_TYPE_PINNED,
    ...
    CUdeviceptr ptr = 0;
    ipc_type ipc_function =
        (ipc_type)dlsym(RTLD_DEFAULT, "cuIpcGetMemHandle");
    multicast_type multicast_function =
        (multicast_type)dlsym(RTLD_DEFAULT, "cuMulticastCreate");

    require(
        cuMemCreate(&handle, 4096, &properties, 0) ==
            CUDA_ERROR_NOT_SUPPORTED,
        "FABRIC create was not rejected");
    require(
        ipc_function != NULL &&
            ipc_function(&ipc, ptr) != CUDA_SUCCESS,
        "legacy IPC was not rejected");
    require(
        multicast_function != NULL &&
            multicast_function(NULL, NULL) ==
                CUDA_ERROR_NOT_SUPPORTED,
        "multicast was not rejected");
}

static void test_fork_exec_contract(void)
{
    CUmemAllocationProp properties = {0};
    CUmemGenericAllocationHandle owner;
    pid_t child;
    ...
    require(
        cuMemCreate(&owner, 4096, &properties, 0) == CUDA_SUCCESS,
        "fork fixture create");
    child = fork();
...
static void test_pre_cuda_fork_contract(void)
{
    ...
    require(
        WIFEXITED(status) && WEXITSTATUS(status) == 0,
        "pre-CUDA fork child could not lazily activate");
}
