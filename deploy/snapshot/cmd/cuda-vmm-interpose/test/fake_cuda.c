...
should_fail(const char* operation)
{
  ...
  for (index = 0; index < sizeof(failpoints) / sizeof(failpoints[0]); index++) {
    if (failpoints[index].operation == NULL || strcmp(failpoints[index].operation, operation) != 0)
      continue;
...
void
fake_cuda_fail_after(const char* operation, long successes)
{
  size_t index;
...
static void
maybe_destroy(struct fake_object* object)
{
...
static struct fake_object*
new_object(size_t size)
{
  size_t index;
...
new_handle(struct fake_object* object)
{
  size_t index;
...
static struct fake_handle*
find_handle(CUmemGenericAllocationHandle value)
{
  size_t index;
...
find_map(CUdeviceptr ptr)
{
  size_t index;

  for (index = 0; index < MAX_MAPS; index++) {
    if (maps[index].used && ptr >= maps[index].ptr && ptr < maps[index].ptr + maps[index].size)
      return &maps[index];
  }
  return NULL;
}
...
static void
mirror_object(struct fake_object* object)
{
  size_t index;
  ...
  for (index = 0; index < MAX_MAPS; index++) {
    if (!maps[index].used || maps[index].object != object)
      continue;
    memcpy((void*)(uintptr_t)maps[index].ptr, object->bytes + maps[index].offset, maps[index].size);
  }
}

static struct fake_object*
object_for_fd(int fd)
{
  struct stat status;
  size_t index;
  ...
  for (index = 0; index < MAX_EXPORTS; index++) {
    if (exports[index].used && exports[index].dev == (uint64_t)status.st_dev &&
...
static struct fake_reservation*
reservation_covering(CUdeviceptr ptr, size_t size)
{
  size_t index;

  for (index = 0; index < MAX_RESERVATIONS; index++) {
    if (reservations[index].used && ptr >= reservations[index].ptr &&
        ptr + size <= reservations[index].ptr + reservations[index].size)
...
void
fake_cuda_reset(void)
{
  size_t index;

  for (index = 0; index < MAX_RESERVATIONS; index++) {
    if (reservations[index].used)
      munmap((void*)(uintptr_t)reservations[index].ptr, reservations[index].size);
  }
  for (index = 0; index < MAX_OBJECTS; index++) free(objects[index].bytes);
...
unsigned int
fake_cuda_create_calls(void)
{
  return create_calls;
}
unsigned int
fake_cuda_granularity_calls(void)
{
  return granularity_calls;
}
unsigned int
fake_cuda_release_calls(void)
{
  return release_calls;
}
unsigned int
fake_cuda_export_calls(void)
{
  return export_calls;
}
unsigned int
fake_cuda_import_calls(void)
{
  return import_calls;
}
unsigned int
fake_cuda_map_calls(void)
{
  return map_calls;
}
unsigned int
fake_cuda_unmap_calls(void)
{
  return unmap_calls;
}
unsigned int
fake_cuda_resolver4_calls(void)
{
  return resolver4_calls;
}
unsigned int
fake_cuda_resolver5_calls(void)
{
  return resolver5_calls;
}
unsigned int
...
unsigned int
fake_cuda_multicast_calls(void)
{
  return multicast_calls;
}
unsigned int
fake_cuda_multicast_operation(void)
{
  return multicast_operation;
}
uint64_t
fake_cuda_multicast_argument(unsigned int index)
{
  return index < 7 ? multicast_arguments[index] : 0;
}
CUmemGenericAllocationHandle
fake_cuda_last_export_handle(void)
{
  return last_export_handle;
}
int
fake_cuda_last_import_fd(void)
{
  return last_import_fd;
}
void*
fake_cuda_last_resolved_entry(void)
{
  return last_resolved_entry;
}

unsigned int
fake_cuda_live_objects(void)
{
  unsigned int result = 0;
  size_t index;
  for (index = 0; index < MAX_OBJECTS; index++) result += objects[index].used;
  return result;
}

size_t
fake_cuda_map_offset(CUdeviceptr ptr)
{
...
unsigned int
fake_cuda_live_references(void)
{
  unsigned int result = 0;
  size_t index;
  for (index = 0; index < MAX_OBJECTS; index++) {
...
unsigned int
fake_cuda_live_handles(void)
{
  unsigned int result = 0;
  size_t index;
  for (index = 0; index < MAX_HANDLES; index++) result += handles[index].used;
  return result;
}

unsigned int
fake_cuda_live_maps(void)
{
  unsigned int result = 0;
  size_t index;
  for (index = 0; index < MAX_MAPS; index++) result += maps[index].used;
  return result;
}

unsigned int
fake_cuda_live_reservations(void)
{
  unsigned int result = 0;
  size_t index;
  for (index = 0; index < MAX_RESERVATIONS; index++) result += reservations[index].used;
  return result;
}
...
cuMemAddressReserve(CUdeviceptr* ptr, size_t size, size_t alignment, CUdeviceptr address, unsigned long long flags)
{
  ...
  memory = mmap(
      (void*)(uintptr_t)address, size, PROT_READ | PROT_WRITE,
      MAP_PRIVATE | MAP_ANONYMOUS | (address != 0 ? MAP_FIXED_NOREPLACE : 0), -1, 0);
...
cuMemAddressFree(CUdeviceptr ptr, size_t size)
{
  ...
  for (index = 0; index < MAX_RESERVATIONS; index++) {
    if (reservations[index].used && reservations[index].ptr == ptr && reservations[index].size == size) {
      if (munmap((void*)(uintptr_t)ptr, size) != 0)
        return CUDA_ERROR_UNKNOWN;
      memset(&reservations[index], 0, sizeof(reservations[index]));
      return CUDA_SUCCESS;
    }
  }
...
CUresult CUDAAPI
cuMemCreate(
    CUmemGenericAllocationHandle* handle, size_t size, const CUmemAllocationProp* properties, unsigned long long flags)
{
  struct fake_object* object;
...
cuMemUnmap(CUdeviceptr ptr, size_t size)
{
  ...
  for (index = 0; index < MAX_MAPS; index++) {
    ...
    if (!maps[index].used || maps[index].ptr != ptr || maps[index].size != size)
      continue;
    object = maps[index].object;
    memcpy(object->bytes + maps[index].offset, (void*)(uintptr_t)ptr, size);
...
static struct fake_access*
access_for(struct fake_map* mapping, const CUmemLocation* location, bool create)
{
  size_t index;

  for (index = 0; index < MAX_ACCESS; index++) {
    if (mapping->access[index].used && mapping->access[index].location.type == location->type &&
...
cuMemSetAccess(CUdeviceptr ptr, size_t size, const CUmemAccessDesc* descriptors, size_t count)
{
  ...
  for (map_index = 0; map_index < MAX_MAPS; map_index++) {
    ...
    if (!maps[map_index].used)
      continue;
    intersection_start = ptr > maps[map_index].ptr ? ptr : maps[map_index].ptr;
    intersection_end =
        end < maps[map_index].ptr + maps[map_index].size ? end : maps[map_index].ptr + maps[map_index].size;
    if (intersection_start >= intersection_end)
      continue;
    for (descriptor = 0; descriptor < count; descriptor++) {
      struct fake_access* access = access_for(&maps[map_index], &descriptors[descriptor].location, true);
      if (access == NULL)
        return CUDA_ERROR_OUT_OF_MEMORY;
...
cuMemGetAccess(unsigned long long* flags, const CUmemLocation* location, CUdeviceptr ptr)
{
  ...
  *flags = access != NULL ? access->flags : CU_MEM_ACCESS_FLAGS_PROT_NONE;
  return CUDA_SUCCESS;
}
...
cuMemcpyHtoD_v2(CUdeviceptr destination, const void* source, size_t size)
{
  ...
  if (should_fail("copy"))
    return CUDA_ERROR_UNKNOWN;
  if (mapping == NULL || destination + size > mapping->ptr + mapping->size)
    return CUDA_ERROR_INVALID_VALUE;
...
cuMemcpyDtoH_v2(void* destination, CUdeviceptr source, size_t size)
{
  ...
  if (should_fail("copy"))
    return CUDA_ERROR_UNKNOWN;
  if (mapping == NULL || source + size > mapping->ptr + mapping->size)
    return CUDA_ERROR_INVALID_VALUE;
    ...
    void* shareable, CUmemGenericAllocationHandle handle, CUmemAllocationHandleType type, unsigned long long flags)
{
  ...
  (void)flags;
  if (type != CU_MEM_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR || reference == NULL)
    return CUDA_ERROR_NOT_SUPPORTED;
...
CUresult CUDAAPI
cuMemImportFromShareableHandle(CUmemGenericAllocationHandle* handle, void* os_handle, CUmemAllocationHandleType type)
{
...
CUresult CUDAAPI
cuMemRetainAllocationHandle(CUmemGenericAllocationHandle* handle, void* address)
{
  struct fake_map* mapping = find_map((CUdeviceptr)(uintptr_t)address);
  struct fake_handle* reference;
...
static CUresult
record_multicast(
    unsigned int operation, uint64_t argument0, uint64_t argument1, uint64_t argument2, uint64_t argument3,
...
cuMulticastCreate(CUmemGenericAllocationHandle* output, const CUmulticastObjectProp* properties)
{
  ...
  return record_multicast(1, (uint64_t)(uintptr_t)output, (uint64_t)(uintptr_t)properties, 0, 0, 0, 0, 0);
}

CUresult CUDAAPI
cuMulticastAddDevice(CUmemGenericAllocationHandle handle, CUdevice device)
{
  return record_multicast(2, handle, (uint64_t)(unsigned int)device, 0, 0, 0, 0, 0);
}
...
{
  return record_multicast(3, multicast, multicast_offset, memory, memory_offset, size, flags, 0);
}
    ...
    CUmemGenericAllocationHandle memory, size_t memory_offset, size_t size, unsigned long long flags)
{
  return record_multicast(
      4, multicast, (uint64_t)(unsigned int)device, multicast_offset, memory, memory_offset, size, flags);
}
...
cuMulticastBindAddr(
    ...
    unsigned long long flags)
{
  return record_multicast(5, multicast, multicast_offset, ptr, size, flags, 0, 0);
}
...
cuMulticastBindAddr_v2(
    ...
    unsigned long long flags)
{
  return record_multicast(6, multicast, (uint64_t)(unsigned int)device, multicast_offset, ptr, size, flags, 0);
}

CUresult CUDAAPI
cuMulticastUnbind(CUmemGenericAllocationHandle multicast, CUdevice device, size_t multicast_offset, size_t size)
{
  return record_multicast(7, multicast, (uint64_t)(unsigned int)device, multicast_offset, size, 0, 0, 0);
}
